...
(function($){
	...
	$.fn.areacomplete = function(obj){
		if( typeof $.support.msie != 'undefined' ) obj.mode = 'outter';
	...
	var browser =  {isChrome: $.support.webkit };
	...
	function onUserSelected(li,data){
		...
		var selectionEnd = getTextAreaSelectionEnd();//.selectionEnd;
		//var text = data.ta.value;
		var text = $(data.ta).text();
		text = text.substr(0,selectionEnd);
		//if( text.charAt(text.length-1) == ' ' || text.charAt(text.length-1) == '\n' ) return "";
		...
		var a = $(data.ta).text().substr(0, pos + 1);
		var c = $(data.ta).text().substr(selectionEnd, $(data.ta).text().length);
		...
		console.log($(data.ta).text())
		console.log(c)
		//$(data.ta).text().replace(/(\s+)?.$/, '');
		
		
		$(data.ta).html().replace(c, '');
		$(data.ta).append(selectedText);
